...
impl BackupContext {
    ...
    pub async fn backup(&self) -> Result<(), FileSystemError> {
        ...
        Self::backup_recursive(backup_url, &self.fs, "/.app", "").await?;
        Self::backup_recursive(backup_url, &self.fs, "", "/bin").await?;
        Self::backup_recursive(backup_url, &self.fs, "", "/lib").await
    }
    ...
    pub async fn restore(&self) -> Result<(), FileSystemError> {
        ...
        Self::restore_recursive(backup_url, &self.fs, "/.app").await?;
        Self::restore_recursive(backup_url, &self.fs, "/bin").await?;
        Self::restore_recursive(backup_url, &self.fs, "/lib").await
    }
                ...
                FileType::Directory => {
                    {
                        let fs = fs
                            ...
                            .try_write()
                            .map_err(|_| FileSystemError::from(FsError::IOError))?;
                        let _ = fs.create_dir(&PathBuf::from(&full_path));
                    }
