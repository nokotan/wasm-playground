...
export function startWorker(module, memory, state, opts, helper) {
    ...
    try {
        worker.postMessage([__webpack_public_path__, module, memory, state, helper.mainJS()]);
    } catch(err) {
...
export function startWasm(module, memory, ctx, opts, helper, wasm_module, wasm_memory) {
    ...
    try {
        worker.postMessage([__webpack_public_path__, module, memory, ctx, helper.mainJS(), wasm_module, wasm_memory]);
    } catch(err) {
...
if (!self.onmessage) {
    ...
    self.onmessage = async event => {
        ...
        if (event.data.length == 5) {
            ...
            __webpack_public_path__ = moduleUrl;
